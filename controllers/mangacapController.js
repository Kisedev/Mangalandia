...
exports.mangacap_add_get = function(req, res, next) {
  manga.findById(req.params.manga_id).exec(function(err, manga) {
    if (err) {
      next(err);
    }
...
exports.mangacap_add_post = [
  body("isbn", "ISBN é necessário")
    .isLength({ min: 1 })
    .trim(),
  body("num", "Número do capítulo é necessário")
    .isLength({ min: 1 })
    .trim(),
  body("lancamento", "Data de lançamento inválida")
  ...
  (req, res, next) => {
    ...
    if (!errors.isEmpty()) {
      manga.findById(req.body.manga).exec((err, manga) => {
        if (err) {
          return next(err);
        }
        res.render("forms/capitulo", {
    ...
    if (!errors.isEmpty()) {
      manga.findById(req.body.manga).exec((err, manga) => {
        ...
        res.render("forms/capitulo", {
          ...
          errors: errors.array(),
          manga,
          capitulo: cap_editado
        });
      });
      return;
    } else {
      mangacap.findByIdAndUpdate(
        req.params.id,
        cap_editado,
        (err, cap_atualizado) => {
          if (err) {
