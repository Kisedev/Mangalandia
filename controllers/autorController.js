  ...
  async.parallel(
    ...
    function(err, results) {
      ...
      res.render("autor", {
        ...
        autor: results.autor,
        autor_mangas: results.autor_mangas
      });
    }
  );
};
...
exports.autor_add_post = [
  body("primeiro_nome")
    ...
    .trim()
    .withMessage("Primeiro nome necessário (max 100 letras)")
    .isAlphanumeric()
    .withMessage("Não deve conter números"),
  body("sobrenome")
    .isLength({ min: 3, max: 100 })
    .trim()
    .withMessage("Sobrenome é necessário (max 100 letras)")
    .isAlphanumeric()
    .withMessage("Não deve conter números"),
  body("nascimento", "Data de nascimento incorreta")
    .optional({ checkFalsy: true })
    .isISO8601(),
  body("falecimento", "Data de falecimento incorreta")
    .optional({ checkFalsy: true })
    .isISO8601(),
  ...
  (req, res, next) => {
    ...
    if (!errors.isEmpty()) {
      res.render("forms/autor", {
        title: "Adicionar Mangaká",
        autor: req.body,
        errors: errors.array()
      });
      return;
  ...
  async.parallel(
    {
      ...
      autor_mangas: function(callback) {
        manga.find({ autor: req.params.id }).exec(callback);
      }
    },
    (err, results) => {
      if (err) {
...
exports.autor_rm_post = function(req, res, next) {
  async.parallel(
    {
      autor: function(callback) {
        autor.findById(req.body.autor_id).exec(callback);
      },
      autor_mangas: function(callback) {
        manga.find({ autor: req.body.autor_id }).exec(callback);
      }
    },
    (err, results) => {
      if (err) {
...
exports.autor_att_get = function(req, res, next) {
  autor.findById(req.params.id).exec((err, autor) => {
    if (err) {
      var err = new Error("Autor não encontrado");
      err.status = 404;
      return next(err);
    }
    res.render("forms/autor", { title: "Atualizar Mangaká", autor });
  });
};

// Atualiza por POST
exports.autor_att_post = [
  body("primeiro_nome")
    .isLength({ min: 3, max: 100 })
    .trim()
    .withMessage("Primeiro nome necessário (max 100 letras)")
  ...
  (req, res, next) => {
    ...
    if (!errors.isEmpty()) {
      res.render("forms/autor", {
        title: "Atualizar Mangaká",
        autor: req.body,
        errors: errors.array()
      });
      return;
    } else {
      let mgk_editado = new autor({
        primeiro_nome: req.body.primeiro_nome,
        sobrenome: req.body.sobrenome,
        nascimento: req.body.nascimento,
        falecimento: req.body.falecimento,
      ...
      autor.findByIdAndUpdate(req.params.id, mgk_editado, (err, mgk_atualizado) => {
        if (err) {return next(err)}
        res.redirect(mgk_atualizado.url);
      })
    }
  }
];
